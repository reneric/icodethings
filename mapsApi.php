...
var map;
var infowindow;
//alert('<?php echo $_SESSION["lat"]; ?>,<?php echo $_SESSION["lng"]; ?>')

bh();
function initialize () {

    inits('<?php echo $_SESSION["lat"]; ?>', '<?php echo $_SESSION["lng"]; ?>');
    codeLatLng('<?php echo $_SESSION["lat"]; ?>', '<?php echo $_SESSION["lng"]; ?>');
    //alert('<?php echo $lat; ?>, <?php echo $lng;?>')
}
...
function codeLatLng(lat,lng) {
    var latlngs = new google.maps.LatLng('<?php echo $_SESSION["lat"]; ?>', '<?php echo $_SESSION["lng"]; ?>');
...
function inits(lat,lng) {
 var latlng = new google.maps.LatLng('<?php echo $_SESSION["lat"]; ?>', '<?php echo $_SESSION["lng"]; ?>');
...
var searchbox = new google.maps.places.SearchBox(input, soptions);
...
var step = "";
    var n = d.getDay(); 
    var bLat = details.geometry.location.lat();
    var bLng = details.geometry.location.lng();
    var content = '<h1 class="titles">' + details.name + '</h1>';
    content += '<h5>'+details.vicinity+'</h5>';
    content += '<div class="clearfix sMap">';
    content += '<img src="http://maps.googleapis.com/maps/api/staticmap?center=' + bLat + ',' + bLng + '&zoom=17&markers=size:mid|' + bLat + ',' + bLng + '&size=330x150&scale=2&visual_refresh=true&maptype=ROADMAP&sensor=false"/>';
    content += '</div>';
    //content += '<input name="direct" type="button" class="steps" value="Directions">';
    content += '<div class="direct">';
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      directionsDisplay.setDirections(response);
      var lent = response.routes[0].legs[0].steps.length;
      for (var i = 0; i < lent; i++) {
        step += '<p>'+response.routes[0].legs[0].steps[i].instructions;
        duration = response.routes[0].legs[0].steps[i].duration.value/60;
        dMin = Math.round( duration * 1 ) / 1;
        if(dMin == "0"){
          dur = "Less than 1 minute";
        }else if(dMin == "1"){
          dur = dMin + " minute";
        }else{
          dur = dMin + " minutes";
        }
        step += '<span>'+dur+'</span></p>';
        step += response.routes[0].legs[0].steps[i].end_location;
}
    content += step;
    content += '</div>';
       minutes = (response.routes[0].legs[0].duration.value)/60;
       minute = Math.round( minutes * 1 ) / 1;
       
    content += '<h2>'+minute+" minutes <span>with traffic.</span></h2>";
    ...
    if(cl > 1200){
     i = (cl-1200);
      var cc = cl.substring(2,4);
      cd = cl.substring(0,2);
      hours = cd-12 + ":";
      min = cl.substring(2,4);
      close = hours;
      close += min;
      close += "pm";
    }else if(cl == 0000){
      ...
      c = cl.substr(1, 4);
      close = c.substring(0,1) + ':' + c.substring(1,3) + "am";
    }else{
      close = c.substring(0,2) + ':' + c.substring(2,4) + "am";
    }
    // content += ' - ' + close;
    var cat = "";
    content += '<span class="closed">'+close+'</span>';
    for (var i = 0; i < place.types.length; i++) {

     cat += '<p>' + place.types[i] + '</p>';

    }
    var stars = '';      
      for (var e = 0; e < 5; e++) {
        if (place.rating < (e + 0.5)) {
          stars += '&#10025;';
        } else {
          stars += '&#10029;';
        }
      }
    content += '<div class="clearfix rating">'+stars+'<span>Rating: '+place.rating+'</span></div>'
    content += '<a href="'+details.website+'">Website</a>';

    $("#places ul").append('<li class="clearfix visible tile">' + content + '</li>');
var handler = null;
        var options = {
           autoResize: true,
           container: $('#tiles'), 
...
function saveCoords(lat,lng) {
  ...
  $.ajax({
    'url': 'http://icodethings.com/wp-content/themes/twentytwelve/saveCoords.php',
    'type': 'post',
    'data': data,
